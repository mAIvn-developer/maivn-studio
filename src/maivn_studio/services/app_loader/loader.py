...
class AppLoader:
    ...
    def __init__(self, base_path: Path | None = None) -> None:
        ...
        self._cache: dict[str, LoadedApp] = {}
    ...
    def load(
        ...
        logger.info(f"Loading app module: {config.module}")
    ...
    def _find_invocation_config(self, module: ModuleType) -> dict[str, Any] | None:
        ...
        """
        raw = getattr(module, "APP_INVOCATION", None)
    ...
    def _find_executor_name(self, module: ModuleType) -> str | None:
        raw = getattr(module, "APP_EXECUTOR", None)
        if raw is None:
            return None
    ...
    def _find_prompts(self, module: ModuleType, config: AppConfig) -> list[AppPrompt]:
        ...
        if hasattr(module, "APP_PROMPTS"):
            app_prompts = module.APP_PROMPTS
            if isinstance(app_prompts, list):
                for i, p in enumerate(app_prompts):
            ...
            if content:
                prompts.append(
                    AppPrompt(
                        ...
                        name=f"Prompt {i + 1}" if len(extractor.contents) > 1 else "App Prompt",
                        content=content,
    ...
    def unload(self, app_id: str) -> None:
        ...
        keys = [key for key in self._cache if key == app_id or key.startswith(f"{app_id}:")]
        for key in keys:
            del self._cache[key]
