"""Data models for loaded demos and prompts."""
...
@dataclass
class DemoPrompt:
    """A pre-defined or saved prompt for a demo."""
...
@dataclass
class LoadedDemo:
    """A loaded and introspected demo module."""
    ...
    def executor(self) -> Agent | Swarm | None:
        ...
        if explicit_name:
            ...
            if resolved is not None:
                return resolved
            logger.warning(
                "Demo %s requested DEMO_EXECUTOR=%r but no matching agent or swarm was found",
                self.config.id,
                explicit_name,
            )
        if self.swarms:
    ...
    def has_executor(self) -> bool:
        """Check if demo has an executable agent or swarm."""
    ...
    def get_default_prompt(self) -> DemoPrompt | None:
    ...
    def get_prompt(self, prompt_id: str) -> DemoPrompt | None:
        """Get a prompt by ID."""
        for prompt in self.prompts:
