...
def test_build_app_config_extracts_metadata(tmp_path) -> None:
...
def test_scan_repo_for_apps_respects_excludes(tmp_path) -> None:
    ...
    assert len(items) == 1
    assert Path(items[0]["file_path"]).as_posix() == "apps/app_one.py"


def test_scan_repo_for_apps_ignores_paths_outside_base(tmp_path) -> None:
