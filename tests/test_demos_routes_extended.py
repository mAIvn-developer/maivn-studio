"""Extended tests for demo API routes covering update endpoints and error cases."""
...
class _Loader:
    ...
    def unload(self, _demo_id: str) -> None:
        pass
...
def _make_app(tmp_path, demo: DemoConfig | None = None):
    if demo is None:
        demo = DemoConfig(
            id="demo-1",
            name="Demo One",
    ...
    return create_app(config=config, base_path=tmp_path), demo


# MARK: Full Details Endpoint


def test_get_demo_full_details_returns_agents_swarms_tools_prompts(monkeypatch, tmp_path) -> None:
    ...
    with TestClient(app) as client:
        resp = client.get("/api/demos/demo-1/details")
        ...
        assert agent["name"] == "demo_agent"
        assert agent["description"] == "Demo agent"
        assert agent["system_prompt"] == "You are a demo agent."
        assert agent["tool_count"] == 1
        assert agent["runtime_tool_count"] == 1
        assert agent["is_swarm_member"] is True
        assert agent["swarm"] == "demo_swarm"

        # Swarms
        assert len(detail["swarms"]) == 1
        swarm = detail["swarms"][0]
        assert swarm["name"] == "demo_swarm"
        assert swarm["agent_count"] == 1
        assert swarm["agent_names"] == ["demo_agent"]
...
def test_get_demo_full_details_runtime_tool_count_excludes_swarm_member_agents(
    monkeypatch, tmp_path
) -> None:
    app, _ = _make_app(tmp_path)
    import maivn_studio.api.routes.demos.routes as demos_routes

    monkeypatch.setattr(demos_routes, "get_demo_loader", lambda: _MixedLoader())
...
def test_get_demo_full_details_not_found(monkeypatch, tmp_path) -> None:
    app, _ = _make_app(tmp_path)
    import maivn_studio.api.routes.demos.routes as demos_routes
    ...
    with TestClient(app) as client:
        resp = client.get("/api/demos/nonexistent/details")
        assert resp.status_code == 404
        assert "not found" in resp.json()["detail"].lower()


# MARK: Update Demo Endpoint


def test_update_demo_name_description_category(monkeypatch, tmp_path) -> None:
    app, _ = _make_app(tmp_path)
    import maivn_studio.api.routes.demos.routes as demos_routes
    ...
    with (
        ...
        with TestClient(app) as client:
            ...
            assert resp.status_code == 200
            data = resp.json()
            assert data["demo"]["name"] == "Updated Demo"
            assert data["demo"]["description"] == "New description"
            assert data["demo"]["category"] == "updated"


def test_update_demo_tags_and_private_data(monkeypatch, tmp_path) -> None:
    app, _ = _make_app(tmp_path)
    import maivn_studio.api.routes.demos.routes as demos_routes
    ...
    with (
        ...
        with TestClient(app) as client:
            ...
            assert resp.status_code == 200
            data = resp.json()
            assert data["demo"]["tags"] == ["new-tag", "another-tag"]
            assert data["demo"]["private_data"] == {"api_key": "secret123"}


def test_update_demo_variants(monkeypatch, tmp_path) -> None:
    app, _ = _make_app(tmp_path)
    import maivn_studio.api.routes.demos.routes as demos_routes
    ...
    mock_config = MagicMock()
    mock_config.demos = [DemoConfig(id="demo-1", name="Demo One", module="demos.demo_one")]
...
def test_update_demo_not_found(monkeypatch, tmp_path) -> None:
    app, _ = _make_app(tmp_path)
    import maivn_studio.api.routes.demos.routes as demos_routes
    ...
    with TestClient(app) as client:
        resp = client.patch("/api/demos/nonexistent", json={"name": "x"})
        assert resp.status_code == 404


def test_update_demo_appends_when_not_found_in_config(monkeypatch, tmp_path) -> None:
    """When the demo exists in the registry but not in config.demos, it should be appended."""
    app, _ = _make_app(tmp_path)
    import maivn_studio.api.routes.demos.routes as demos_routes
    ...
    mock_config = MagicMock()
    # Config has a different demo, so for-else will append
    mock_config.demos = [DemoConfig(id="other-demo", name="Other", module="demos.other")]
    ...
    with (
        ...
        with TestClient(app) as client:
            resp = client.patch(
                "/api/demos/demo-1",
                json={"name": "Appended Demo"},
            )
            assert resp.status_code == 200
            assert resp.json()["demo"]["name"] == "Appended Demo"
            # Verify the demo was appended
            assert len(mock_config.demos) == 2
...
def test_update_agent_not_found(monkeypatch, tmp_path) -> None:
    ...
    with TestClient(app) as client:
        resp = client.patch(
            "/api/demos/demo-1/agents/nonexistent_agent",
            json={"description": "x"},
        )
        assert resp.status_code == 404
...
def test_update_agent_demo_not_found(monkeypatch, tmp_path) -> None:
    app, _ = _make_app(tmp_path)
    import maivn_studio.api.routes.demos.routes as demos_routes
    ...
    with TestClient(app) as client:
        resp = client.patch(
            "/api/demos/nonexistent/agents/demo_agent",
            json={"description": "x"},
        )
        assert resp.status_code == 404
        assert "Demo not found" in resp.json()["detail"]
...
def test_update_swarm_fields(monkeypatch, tmp_path) -> None:
    ...
    with TestClient(app) as client:
        resp = client.patch(
            "/api/demos/demo-1/swarms/demo_swarm",
            json={
...
def test_update_swarm_not_found(monkeypatch, tmp_path) -> None:
    ...
    with TestClient(app) as client:
        resp = client.patch(
            "/api/demos/demo-1/swarms/nonexistent_swarm",
            json={"description": "x"},
        )
        assert resp.status_code == 404
...
def test_update_swarm_demo_not_found(monkeypatch, tmp_path) -> None:
    app, _ = _make_app(tmp_path)
    import maivn_studio.api.routes.demos.routes as demos_routes
    ...
    with TestClient(app) as client:
        resp = client.patch(
            "/api/demos/nonexistent/swarms/demo_swarm",
            json={"description": "x"},
        )
        assert resp.status_code == 404
        assert "Demo not found" in resp.json()["detail"]
