...
from maivn_studio.services.studio_reporter.reporter.streaming import compute_stream_delta
...
class TestNoOpMethods:
    ...
    async def test_print_summary(self) -> None:
        _, reporter = _make_pair()
        assert reporter.print_summary(TokenUsage(total_tokens=100)) is None
...
def _respond_with_chad(_interrupt_id: str) -> str:
    """Stub for ``get_interrupt_response`` that always returns a fixed answer."""
    return "Chad"


def _cleanup_noop(_interrupt_id: str) -> None:
    """Stub for ``cleanup_interrupt`` that does nothing."""
    return None
...
class TestInterruptInput:
    ...
    async def test_get_input_registers_session_scoped_alias_for_data_key(
        ...
        monkeypatch.setattr(
            "maivn_studio.services.studio_reporter.interrupts.register_interrupt",
            _register_interrupt,
        )
        monkeypatch.setattr(
            "maivn_studio.services.studio_reporter.interrupts.get_interrupt_response",
            _respond_with_chad,
        )
        monkeypatch.setattr(
            "maivn_studio.services.studio_reporter.interrupts.cleanup_interrupt",
            _cleanup_noop,
        )
...
class TestReportSystemToolProgress:
    ...
    async def test_no_delta_emitted_for_duplicate(self) -> None:
        bridge, reporter = _make_pair()
        reporter.stream_text_by_tool_id["st-2"] = ""
    ...
    async def test_contract_stream_mode_skips_system_tool_progress_state_updates(self) -> None:
        bridge, reporter = _make_pair()
        reporter.stream_text_by_tool_id["st-3"] = "existing"
        token = current_sdk_delivery_mode.set("stream")
        try:
            reporter.report_system_tool_progress(
                event_id="st-3",
        ...
        assert reporter.stream_text_by_tool_id["st-3"] == "existing"
        chunks = _events_of_type(bridge, "system_tool_chunk")
        assert chunks == []
...
class TestReportResponseChunk:
    ...
    async def test_contract_stream_mode_skips_response_chunk_state_updates(self) -> None:
        bridge, reporter = _make_pair()
        reporter.response_stream_text_by_assistant_id["a1"] = "existing"
        token = current_sdk_delivery_mode.set("stream")
        try:
        ...
        assert reporter.response_stream_text_by_assistant_id["a1"] == "existing"
        chunks = _events_of_type(bridge, "assistant_chunk")
        assert chunks == []
    ...
    async def test_contract_stream_mode_skips_final_response_state_updates(self) -> None:
        bridge, reporter = _make_pair()
        reporter.response_stream_text_by_assistant_id["assistant"] = "existing"
        token = current_sdk_delivery_mode.set("stream")
        try:
        ...
        assert reporter.response_stream_text_by_assistant_id["assistant"] == "existing"
        chunks = _events_of_type(bridge, "assistant_chunk")
        assert chunks == []


# MARK: compute_stream_delta


class TestComputeStreamDelta:
    def test_empty_incoming(self) -> None:
        assert compute_stream_delta("abc", "") == ""

    def test_empty_previous(self) -> None:
        assert compute_stream_delta("", "hello") == "hello"

    def test_incoming_extends_previous(self) -> None:
        assert compute_stream_delta("Hello", "Hello World") == " World"

    def test_previous_extends_incoming(self) -> None:
        assert compute_stream_delta("Hello World", "Hello") == ""

    def test_diverged_text(self) -> None:
        result = compute_stream_delta("ABC", "ABX")
        assert result == "X"

    def test_completely_different(self) -> None:
        result = compute_stream_delta("abc", "XYZ")
        assert result == "XYZ"

    def test_both_empty(self) -> None:
        assert compute_stream_delta("", "") == ""
