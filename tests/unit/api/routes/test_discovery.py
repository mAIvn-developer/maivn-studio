...
def test_discovery_scan_and_apply(tmp_path: Path) -> None:
    ...
    with TestClient(app) as client:
        ...
        scan_data: dict[str, Any] = scan_resp.json()
        assert scan_data["total"] == 1

        first_item: dict[str, Any] = scan_data["items"][0]
        ...
        apply_data: dict[str, Any] = apply_resp.json()
...
def test_discovery_apply_ignores_files_outside_base_path(tmp_path: Path) -> None:
