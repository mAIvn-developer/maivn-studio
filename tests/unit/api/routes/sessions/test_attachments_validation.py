...
def _no_event_bridge(_session_id: str) -> None:
    return None


def _noop_set_config(_config: StudioConfig, _config_path: Path | None) -> None:
    return None
...
async def test_create_session_maps_attachment_value_error_to_http_422(
    ...
    monkeypatch.setattr(sessions_writes, "create_event_bridge", _no_event_bridge)
...
async def test_create_session_refreshes_registry_private_data_from_disk(
    ...
    monkeypatch.setattr(sessions_helpers, "set_config", _noop_set_config)

    def _init_registry(config: StudioConfig, _base_path: Path) -> None:
        registry.app = config.apps[0]
